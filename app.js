...
/**
 * 예시: member_id를 기반으로 고객 개인정보 데이터를 가져옵니다.
 * 쿼리 파라미터로 member_id가 제공되지 않으면 기본값 'sampleid'를 사용합니다.
 */
async function getCustomerDataByMemberId(memberId) {
  ...
  try {
    ...
    console.log('Customer Privacy Data:', JSON.stringify(data, null, 2));
    return data;
  } catch (error) {
    console.error(`Error fetching customer privacy data for member_id ${memberId}:`, error);
    throw error;
  }
}
  ...
  .then(() => {
    ...
    app.post('/api/entry', async (req, res) => {
      ...
      try {
        ...
        // 쿼리 파라미터가 없으면 기본값 'sampleid' 사용
        const effectiveMemberId = memberId || 'sampleid';
        const customerData = await getCustomerDataByMemberId(effectiveMemberId);
        // customersprivacy 객체가 바로 반환되는 경우라면 아래처럼 사용합니다.
    ...
    // 쿼리 파라미터로 전달된 member_id가 없으면 기본값 'sampleid'를 사용합니다.
    app.get('/api/customer', async (req, res) => {
      const memberId = req.query.member_id || 'sampleid';
      try {
